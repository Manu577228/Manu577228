<div align="center">

<img src="https://i.postimg.cc/SKBNcms1/Bharadwaj-removebg-preview.png"
width="180"
style="border-radius:16px; box-shadow:0 0 35px #000000aa;" />

# Manu Bharadwaj

**Software Engineer • Competitive Programmer • Scalable Systems**

<img src="https://readme-typing-svg.herokuapp.com?font=Courier&size=20&duration=2800&color=58A6FF&center=true&vCenter=true&width=850&lines=Global+Rank+50+LeetCode;Advanced+Competitive+Programmer;Performance-First+Engineering;Designing+Systems+That+Scale" />

<br/>

<a href="https://www.linkedin.com/in/manu-bharadwaj-3507a345">
<img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=58A6FF"/>
</a>
<a href="https://github.com/Manu577228">
<img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=58A6FF"/>
</a>
<a href="https://leetcode.com/u/mbbn/">
<img src="https://img.shields.io/badge/LeetCode-0D1117?style=for-the-badge&logo=leetcode&logoColor=58A6FF"/>
</a>
<a href="https://codeforces.com/profile/MBBN">
<img src="https://img.shields.io/badge/Codeforces-0D1117?style=for-the-badge&logo=codeforces&logoColor=58A6FF"/>
</a>

</div>

---

## PROFILE

Engineer with **2.8+ years** building production systems — backed by strong competitive programming fundamentals.

- Scalable architecture  
- Low-latency distributed APIs  
- Modular frontend systems  
- Performance optimization  
- Algorithmic rigor  

---

## COMPETITIVE PROGRAMMING

- 🌍 **Global Rank 49 — LeetCode**
- Advanced problem-solving across graphs, DP, trees, greedy, and data structures
- Strong contest performance under strict time constraints
- Deep understanding of time & space complexity tradeoffs
- Consistent high-performance implementations in Java.

<div align="center">

<img src="https://leetcard.jacoblin.cool/mbbn?theme=dark&font=Courier&ext=contest"/>

<br/>

<img src="https://codeforces-readme-stats.vercel.app/api/card?username=MBBN"/>

</div>

---

## CORE STACK

**Frontend**  
React • Angular • Next.js • TypeScript • Javascript • Redux  

**Backend**  
• Spring Boot • Express.js • Django • Kafka • Docker

**Data**  
PostgreSQL • MySQL • MongoDB • Redis  

**Infra**  
Docker • CI/CD • Cloud-native workflows  

---

## SIGNATURE WORK

**Scalable URL Platform**  
High-concurrency redirection engine with optimized indexing & low-latency architecture.

**Intelligent Expense System**  
Optimized settlement computation engine with secure multi-user workflows.

---

## PRINCIPLES

Algorithmic clarity.  
Measure before optimizing.  
Design for scale.  
Own outcomes end-to-end.

---

📧 bnmanubharadwaj@gmail.com  
🌐 https://manu-bharadwaj-portfolio.vercel.app/

<div align="center">

**Competitive precision. Scalable engineering. Measurable results.**

</div>
